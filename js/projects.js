...
projectCards.forEach(card => {
    ...
    const companyInfoLink    = document.getElementById('companyInfoLink');
    ...
    const projectHistory = {
        ...
        "017_JMETK": 0,
        "018_PCOOP": 0,
        "019_ERRTM": "https://www.skyscrapercity.com/threads/tallinn-err-s-new-tv-building-err-i-uus-telemaja-u-c.2431574/?post_id=191686222#post-191686222"
    };
    ...
    const projectVideo = {
        ...
        "001_IL": [
            {
                title: "Ingmar Arch video",
                url: "https://drive.google.com/file/d/1P5N27OUiKsPKOYTi01UWlhSXRNohLcGE/view?usp=sharing"
            }
        ],
        ...
        "002_KR": [
            {
                title: "Ingmar Arch video",
                url: "https://drive.google.com/file/d/1tIqpg_TVqx3Yu2GuTW2yF_Y51ohHp_mb/view?usp=drive_link"
            }
        ],
        ...
        "003_BY": [
            {
                title: "Ingmar Arch video",
                url: "https://drive.google.com/file/d/1GvoF-6E10CFrEojiPkfNwNTajDZVA-FV/view?usp=drive_link"
            }
        ],
        ...
        "014_KASTANI": [
            {
                title: "Dorpat video, 09.02.2026",
                url: "014_KASTANI/014_KASTANI_01 (720).mp4"
            },

            {
                title: "Dorpat video, 07.04.2026",
                url: "014_KASTANI/014_KASTANI_02.mp4"
            }
        ],
        ...
        "019_ERRTM": [
            {
                title: "ERR video",
                url: "https://vod.err.ee/file/etvvideod/1b1d87ef7976422c0e8441468c86da58.mp4"
            }
        ]
    };
    ...
    let companyWarningUrl = "";

    function showCompanyWarning(url) {

        const modal =
            document.getElementById('companyWarningModal');
        ...
        companyWarningUrl = url;

        modal.classList.add('open');
    }


    function hasCompanyWarning() {

        /* Если выбран конкретный проект */
        if (currentProjectCard) {

            if (currentProjectCard.dataset.warning === "true") {
                return true;
            }

        }

        /* Если выбран фильтр компании */
        if (companyFilter) {
            ...
            return Array.from(projectCards).some(card =>
                card.dataset.companyId === selectedCompany &&
                card.dataset.warning === "true"
            );
    ...
    const companyWarningOk =
        document.getElementById('companyWarningOk');

    if (companyWarningOk) {

        companyWarningOk.addEventListener('click', () => {
            ...
            if (companyWarningUrl) {
                window.open(companyWarningUrl, '_blank');
            }
    ...
    function updateButtons() {
        ...
        if (companyFilter && companySiteBtn) {
            ...
            const selectedOption =
                companyFilter.options[companyFilter.selectedIndex];
            ...
            const dropdown =
                companySiteBtn.closest('.company-dropdown');
        ...
        else if (website && information) {

            enableButton(companySiteBtn);

            companyWebsiteLink.href = website;
            companyWebsiteLink.style.display = '';

            companyInfoLink.href = information;
            companyInfoLink.style.display = '';

            companyMenu.style.display = '';

            companySiteBtn.onclick = (e) => {
            ...
            companyWebsiteLink.onclick = (e) => {

                if (hasCompanyWarning()) {

                    e.preventDefault();
                    e.stopPropagation();

                    showCompanyWarning(website);
    ...
    function updateVideoButton(projectCode) {
        ...
        const videos = projectVideo[projectCode];

        videoMenu.innerHTML = "";

        /* no videos */

        if (!videos || videos === 0 || videos.length === 0) {
        ...
        link.className = "video-menu-item";
...
videoMenu.appendChild(link);
        ...
        videoBtn.onclick = (e) => {
            ...
            const dropdown =
                videoBtn.closest('.video-dropdown');
...
function filterGalleryImages() {
    ...
    document.addEventListener('click', (e) => {
        ...
        const videoDropdown =
            document.querySelector('.video-dropdown');

        const companyDropdown =
            document.querySelector('.company-dropdown');


        if (videoDropdown &&
            !videoDropdown.contains(e.target)) {
